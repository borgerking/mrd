...
<head>
	...
	<script type="text/javascript">
		$(document).ready(function() {
			$("#btnExport").click(function(){
				top.$.jBox.confirm("确认要导出交易数据吗？","系统提示",function(v,h,f){
					if(v=="ok"){
						$("#searchForm").attr("action","${ctx}/jy/transactionDetail/export");
						$("#searchForm").submit();
					}
				},{buttonsFocus:1});
		...
		function page(n,s){
			$("#pageNo").val(n);
			$("#pageSize").val(s);
			$("#searchForm").attr("action","${ctx}/jy/myTransaction/list");
			$("#searchForm").submit();
        	return false;
        }
	</script>
...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/jy/myTransaction/">我的交易查询列表</a></li>
	</ul>
	<form:form id="searchForm" modelAttribute="jyTransaction" action="${ctx}/jy/myTransaction/" method="post" class="breadcrumb form-search">
